...
- [ ] **Step 1: validateProjectDir ヘルパーを抽出**

`runContainer`（docker.go）の「`:` チェック + ホーム露出ガード」ブロックと同じ検査を
cmdSSHRegister でも行う。逐語重複を避けるため、まず docker.go にヘルパーを抽出する:

```go
// validateProjectDir は pwd をマウントしてよいか検査する（':' チェックとホーム露出ガード）。
// 使い捨て実行（runContainer）と SSH 登録（cmdSSHRegister）で共用する。
...
`runContainer` の該当ブロック（`if strings.Contains(pwd, ":")` から警告出力まで）を
`validateProjectDir(pwd, home)` の呼び出しに置き換える。既存テスト
`TestRunContainerRejectsColonInPwd` 等が挙動不変を保証する。

Run: `go test ./... -v`
Expected: PASS（既存テストが全て通る = 挙動不変）

- [ ] **Step 2: cmdSSHRegister を実装**
...
```bash
git add ssh.go sshcontainer.go docker.go main.go
